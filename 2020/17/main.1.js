...
class Cube {
  ...
  generateNeighboringKeys() {
    ...
    for (let z = -1; z <= 1; z++) {
      for (let y = -1; y <= 1; y++) {
        for (let x = -1; x <= 1; x++) {
          const key = `${this.dimensions.x + x}.${this.dimensions.y + y}.${this.dimensions.z + z}`;
          if (key !== this.key) {
            keys.push(`${this.dimensions.x + x}.${this.dimensions.y + y}.${this.dimensions.z + z}`)
          }
        }
      }
    }
...
const cycle = () => {
  Object.values(cubes)
    .forEach(cube => cube.generateNeighboringKeys());
  Object.values(cubes)
    .forEach(cube => {
